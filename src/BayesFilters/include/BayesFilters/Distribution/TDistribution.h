...
class TDistribution
    : public Moment<VariateType>,
      public Evaluation<VariateType>,
      public StandardNormalDistributionMap<
                VariateType,
                JoinSizes<SizeOf<VariateType>::Value, 1>::Value>
{
private:
    typedef StandardNormalDistributionMap<
                VariateType,
                JoinSizes<SizeOf<VariateType>::Value, 1>::Value
...
public:
    ...
    /**
     * \brief Represents the StandardNormalDistributionMap standard variate type which
     *        is of the same dimension as the \c TDistribution \c VariateType. The
...
public:
    ...
    /**
     * \brief Returns a t-distribution sample of the type \c VariateType determined
     * by mapping a standard normal sample into the t-distribution sample space
     *
     * \param sample    Standard normal sample
     *
     * \throws See Gaussian<VariateType>::map_standard_normal
     */
    VariateType map_standard_normal(const StandardVariate& sample) const override
    {
        assert(sample.size() == dimension() + 1);

        Real u = chi2_.map_standard_normal(sample.bottomRows(1)(0));
        VariateType n = normal_.map_standard_normal(sample.topRows(dimension()));

        // rvo
        VariateType v = location() + std::sqrt(degrees_of_freedom() / u) * n;
        return v;
    }
     ...
     *
     * \throws See Gaussian<VariateType>::has_full_rank()
     */
    Real log_probability(const VariateType& x) const override
    {
     ...
     */
    const VariateType& mean() const override
    {
        return location();
    }
     ...
     * (covariance matrix)
     *
     * \throws See Gaussian<VariateType>::covariance()
     */
     ...
     */
    virtual const VariateType& location() const
    {
        return normal_.mean();
    }
     ...
     */
    virtual void location(const VariateType& new_location) noexcept
    {
...
private:
    ...
    public:
        ...
        Real log_probability(
                const TDistribution<VariateType>& t_distr, const VariateType& x)
        {
            if (dirty_) update(t_distr);

            VariateType z = x - t_distr.location();
    ...
    private:
        void update(const TDistribution<VariateType>& t_distr)
        {
