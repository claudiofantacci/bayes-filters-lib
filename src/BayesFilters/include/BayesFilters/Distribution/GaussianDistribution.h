...
public:
     ...
     *                  of the VariateType at compile time is fixed, this will be
     *                  adapted. For dynamic-sized Variable the dimension is
     *                  initialized to 0.
     */
    explicit Gaussian(int dim = DimensionOf<VariateType>()):
     ...
     * \throws see has_full_rank()
     */
    Real log_probability(const VariateType& vector) const override
    {
    ...
    /**
     * \return a Gaussian sample of the type \c VariateType determined by mapping a
