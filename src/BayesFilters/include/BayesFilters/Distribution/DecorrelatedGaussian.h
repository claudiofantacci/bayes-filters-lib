...
class DecorrelatedGaussian
    : public Moment<VariateType, typename DiagonalSecondMomentOf<VariateType>::type>,
      public Evaluation<VariateType>,
      public StandardNormalDistributionMap<VariateType, SizeOf<VariateType>::Value>
{
public:
    typedef Evaluation<VariateType> EvaluationInterface;

    typedef Moment<
                VariateType, typename DiagonalSecondMomentOf<VariateType>::type
            > MomentsInterface;

    typedef StandardNormalDistributionMap<
                VariateType, SizeOf<VariateType>::Value
     ...
     */
    typedef typename DiagonalSecondMomentOf<VariateType>::type DiagonalSecondMoment;

    typedef typename SecondMomentOf<VariateType>::type DenseSecondMoment;

    /**
     * \brief Represents the StandardNormalDistributionMap standard variate type which
     *        is of the same dimension as the Gaussian VariateType. The
...
public:
     ...
     *                  initialized to 0.
     */
    explicit DecorrelatedGaussian(int dim = DimensionOf<VariateType>()):
        StdGaussianMappingInterface(dim)
    {
        static_assert(SizeOf<VariateType>::Value != 0, "Illegal static dimension");
     ...
     * \endcond
     */
    virtual Real log_probability(const VariateType& vector) const
    {
        if(has_full_rank())
        {
     ...
     * \endcond
     */
    virtual VariateType map_standard_normal(const StandardVariate& sample) const
    {
    ...
    virtual void set_standard()
    {
        ...
        mean(VariateType::Zero(dimension()));
     ...
     */
    virtual void mean(const VariateType& mean) noexcept
    {
...
protected:
    /** \cond internal */
    VariateType mean_;                            /**< \brief first moment vector */
